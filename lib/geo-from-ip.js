...
const DBNAME = path.resolve(path.join(__dirname, '/../mmdb/GeoLite2-City.mmdb'));
const DBNAME_EMPTY = path.resolve(path.join(__dirname, '/../mmdb/GeoLite2-City-Zero.mmdb'));
const dbname = fs.existsSync(DBNAME) ? DBNAME : DBNAME_EMPTY;
const mmdb = mmdbreader.openSync(dbname)
...
function allData(ip) {
  ...
  try {
    // get all data for ip
    const geodata = mmdb.getGeoDataSync(ip)
